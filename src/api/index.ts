...
export default class ConnexService {
  ...
  private static async signingService<T>(
    ...
    try {
      ...
      const response = await service.request([{ ...asClause }]);
  ...
  public static async login() {
    try {
      if ('thor' in connex) {
        ...
        const response = await connex.vendor.sign('cert').request({
  ...
  public static async getBallot(id: string, signer: string) {
    try {
      ...
      if (!response || response.data === '0x' || response.decoded?.revertReason) {
        return undefined;
      }
  ...
  public static async castEndTime(voteID: string) {
    try {
      ...
      if (!response || response.data === '0x' || response.decoded?.revertReason || !response.decoded) {
        return undefined;
      }
  ...
  public static async Tally(signer: string, data: Array<string| string[] | number>) {
    try {
      ...
      const response = await service.request([{ ...setTallyResAsClause }, { ...endTallyAsClause }]);
