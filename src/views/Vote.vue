<template>
  <Main className='zkp-vote' :class="[!isTestNet ? 'zkpvote' : '']">
...
export default class Vote extends Vue {
  ...
  private get isPreLoading() {
    return this.proposedBy && this.proposedBy !== '0x' && this.isTestNet && !this.topicDetail.topic;
  }
  ...
  private async getVoteStatus() {
    try {
      ...
      if (this.status === 1 && this.endTime * 1000 < new Date().getTime()) {
        this.status = 2;
      }
  ...
  private async getVoteResult() {
    const voteResult = await ConnexService.getBallot(this.voteID, this.signer);
    if (voteResult) {
  ...
  private async setPublicKey() {
    try {
      ...
      const response = await ConnexService.setAuthPubKey(
        this.signer,
        [this.voteID, toHex(gk.getX()), gk.getY().isEven() ? '0x02' : '0x03'],
      );
...
.zkpvote {
  ...
  max-width: 100%;
  min-height: 800px;
  background: url('../assets/images/not-found/wrong-enviroment-bg.png') no-repeat bottom center;
